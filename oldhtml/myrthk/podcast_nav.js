...
.map(podcast => `			   <li><a href="podcast.html?prog=${pid}&page=${podcast.title.replace(/ /g,'_')}">${podcast.title} (${podcast.episodes})</a></li>`).join('\n');
...
function myInitChapters() {
  ...
  fetch(`${prog}.json`)
    ...
    .then(data => {
      Programme = data;
      Page = Programme.pages.filter(page => page.title.replace(/ /g,'_') === pageTitle)[0]; 
