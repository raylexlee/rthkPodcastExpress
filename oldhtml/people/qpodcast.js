...
.map(podcast => `			   <li><a href="qpodcast.html?prog=${pid}&page=${podcast.title}">${podcast.title} (${podcast.episodes})</a></li>`).join('\n');
  ...
  fetch(`${prog}.json`)
    ...
    .then(data => {
      ...
      if (Programme.pages.length === 1) {
        window.location = `qpodcast.html?prog=${prog}&page=${Programme.pages[0].title}`;
      }
